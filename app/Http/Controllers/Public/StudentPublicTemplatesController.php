...
class StudentPublicTemplatesController extends Controller
{
    ...
    public function templateDetails(Request $request, int $templateAssignmentId, GymShareTokenValidator $validator): JsonResponse
    {
        $token = $request->query('token');
        $dni = null;
        ...
        if (!$user) {
            $baseUrl = config('services.vmserver.base_url');
            $vmserverToken = config('services.vmserver.internal_token') ?: config('services.vmserver.token');
            $timeout = (int) (config('services.vmserver.timeout') ?: 8);

            if ($baseUrl && $vmserverToken) {
                $url = rtrim($baseUrl, '/') . '/api/internal/users/by-dni/' . $dni;

                try {
                    $resp = Http::withToken($vmserverToken)->timeout($timeout)->get($url);
                } catch (\Throwable $e) {
                    return response()->json([
                        'ok' => false,
                        'message' => 'Upstream server error',
        ...
        $assignmentBelongsToStudent = TemplateAssignment::query()
            ->whereKey($templateAssignmentId)
            ->whereHas('professorStudentAssignment', function ($query) use ($user) {
                $query->where('student_id', $user->id);
            })
            ->exists();

        if (!$assignmentBelongsToStudent) {
            return response()->json([
                'ok' => false,
    ...
    public function myWeeklyCalendar(Request $request, GymShareTokenValidator $validator): JsonResponse
    {
        $token = $request->query('token');
        $dni = null;
        ...
        /** @var JsonResponse $response */
        $response = app()->call([$assignmentController, 'myWeeklyCalendar'], ['request' => $internalRequest]);
